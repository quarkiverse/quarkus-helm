...
[TIP]
====
The `<chart name>` is set from either the property `quarkus.helm.name` or the Quarkus application.
====
...
[source,properties]
----
quarkus.helm.expressions.0.path=(kind == Service && metadata.name == quarkus-helm-it-kubernetes-with-templates).metadata.labels
...
**Note**: At the moment, OCI Helm Repositories like Amazon ECR, Azure Container Registry, ... are not supported. https://github.com/quarkiverse/quarkus-helm/issues/371[#371] for future reference and support.
