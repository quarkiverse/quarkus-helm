...
The first thing we need to do is to map the system properties we used to configure the datasource xref:example-crud.adoc#datasource-configuration[here] to the pod container environment properties:

[source,properties]
----
quarkus.kubernetes.env.vars.POSTGRESQL_URL=any
quarkus.kubernetes.env.vars.POSTGRESQL_USERNAME=any
quarkus.kubernetes.env.vars.POSTGRESQL_PASSWORD=any
----

If you build your application again using `mvn clean pacakge`, the Kubernetes resources will be generated at `target/kubernetes/kubernetes.yml` which will contain a Deployment resource with these environment variables:
      ...
      containers:
        - env:
            ...
            - name: POSTGRESQL_USERNAME
              value: any
          image: quay.io/myuser/demo:0.0.1
...
And if you go to the generated Helm chart at `target/helm/kubernetes/demo`, you will see that the file `values.yaml` is also mapping these variables:
