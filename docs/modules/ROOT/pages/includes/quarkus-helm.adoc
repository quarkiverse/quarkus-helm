...
[.description]
--
The output folder in which to place the Helm generated folder. The extension will always create a folder within this location named "helm". By default, if not set, it will use the default value for the property "quarkus.package.output-directory".
...
Environment variable: `+++QUARKUS_HELM_VALUES__VALUES__JSON_PATHS+++`
--|list of string 
|required icon:exclamation-circle[title=Configuration property is required]
